@php
    ...
    $paginatedDivisions = new \Illuminate\Pagination\LengthAwarePaginator(
        $divisions->forPage($currentPage, $itemsPerPage),
        $divisions->count(),
        $itemsPerPage,
        $currentPage,
        [
...
<div class="w-full bg-white rounded-xs border border-[#ebebeb]">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 px-5 py-2 border-b border-[#ebebeb]">
        ...
        <div class="flex items-center gap-3 flex-wrap">
            ...
            <button
                class="h-8 lg:h-10 px-3 rounded-[8px] border border-[#dddddd] bg-red-500 text-xs font-[500] text-white flex items-center gap-2 cursor-pointer" id="add-division-btn"
            >
        ...
        <table class="min-w-[1250px] w-full whitespace-nowrap">
            ...
            <thead class="bg-[#fafafa] border-b border-[#ebebeb]">
                ...
                <tr>
                    ...
                    <th class="px-5 py-2 text-left text-[13px] font-[700] text-[#111111]">Description</th>
            ...
            <tbody>
                ...
                @foreach ($paginatedDivisions as $index => $item)

                    <tr class="border-b border-[#ebebeb] hover:bg-[#fafafa]">

                        <td class="px-5 py-2 text-xs text-[#111111]">
                            {{ ($paginatedDivisions->currentPage() - 1) * $paginatedDivisions->perPage() + $index + 1 }}
                        </td>
                        ...
                        <td class="px-5 py-2">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <button
                                    class="w-9 h-9 rounded-lg border border-[#e5e5e5] flex items-center justify-center hover:bg-[#f5f5f5] cursor-pointer edit-division-btn"  data-id="{{ $item['id'] }}"
                                >
                                    <x-icons.edit-icon class="size-5 text-red-500"/>
                                </button>
                                
                                <button
                                class="w-9 h-9 rounded-lg border border-[#e5e5e5] text-red-500 flex items-center justify-center hover:bg-[#f5f5f5] cursor-pointer delete-division-btn" data-id="{{ $item['id'] }}"
                                >
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5 px-5 py-2">
        ...
        <p class="text-xs text-[#666666]">
            ...
            Showing
            {{ $paginatedDivisions->firstItem() }}
            to
            {{ $paginatedDivisions->lastItem() }}
            of
            {{ $paginatedDivisions->total() }}
            entries

        </p>

        <div>
            {{ $paginatedDivisions->links() }}
        </div>
...
@vite(['resources/js/admin/division/create.js', 'resources/js/admin/division/edit.js', 'resources/js/admin/division/delete.js'])
